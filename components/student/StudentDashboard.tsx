...
import { 
  ...
  Eye, Users, Plus, UserPlus, LogOut, MessageSquare, MonitorPlay, ChevronLeft, ChevronRight, Calendar,
...
export const StudentDashboard: React.FC<StudentDashboardProps> = ({ activeTab, user, onUpdateUser }) => {
  ...
  if (activeTab === 'tasks') {
    // 映射任务ID到中文标题
    const taskSteps = [
        { id: 't1', stepLabel: '任务一', title: '调查主题确认' },
        { id: 't2', stepLabel: '任务二', title: '过程性材料' },
        { id: 't3', stepLabel: '任务三', title: '调查报告' }
    ];
    ...
    return (
      <div className="space-y-8 max-w-5xl mx-auto">
        ...
        {/* 流程箭头导航条 */}
        <div className="flex items-center w-full gap-2 md:gap-4 overflow-x-auto pb-4 md:pb-0">
            {taskSteps.map((step, index) => {
                const isActive = step.id === selectedTaskId;
                const taskInfo = tasks.find(t => t.id === step.id);
                const isCompleted = taskInfo?.status === TaskStatus.APPROVED || taskInfo?.status === TaskStatus.SUBMITTED;
                
                return (
                    <React.Fragment key={step.id}>
                        <button 
                            onClick={() => setSelectedTaskId(step.id)}
                            className={`flex-1 min-w-[200px] group relative p-4 rounded-xl transition-all border-2 text-left flex flex-col justify-center gap-1
                                ${isActive 
                        ...
                        >
                            <div className="flex items-center justify-between w-full">
                                <span className={`text-xs font-bold uppercase tracking-wider ${isActive ? 'text-brand-600' : 'text-slate-400'}`}>
                                    {step.stepLabel}
                                </span>
                                {isCompleted && <CheckCircle2 size={16} className="text-emerald-500" />}
                            </div>
                            <span className={`text-lg font-bold truncate ${isActive ? 'text-brand-800' : 'text-slate-600'}`}>
                                {step.title}
                            </span>
                        </button>
                        ...
                        {index < taskSteps.length - 1 && (
                            <ChevronRight size={24} className="text-slate-300 flex-shrink-0" />
                        )}
                    </React.Fragment>
                );
            })}
        ...
        <div className="grid grid-cols-1 gap-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
          {tasks.map((task) => {
            if (task.id !== selectedTaskId) return null; // 只渲染当前选中的任务
